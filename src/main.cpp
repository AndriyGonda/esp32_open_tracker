...
void setup() {
    ...
    wifiConnected = wifiConnector.connectToFirstAvailableSavedNetwork();

    tracker.begin();

    if (!wifiConnected) {
        Serial.println("Could not connect to saved WiFi.");
        Serial.println("Hold button on GPIO10 to open config portal.");
        wifiConnector.markLastReconnectNow();
    }
}
