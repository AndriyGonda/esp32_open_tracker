...
void Tracker::syncTimeNTP() {
    ...
    if (now <= 0) {
        Serial.println("[Tracker] NTP sync failed: invalid epoch");
        return;
    }

    lastValidUnixTime = (unsigned long)now;
    timeSynced = true;

    Serial.print("[Tracker] NTP synced UTC: ");
...
unsigned long Tracker::getSafeTimestamp(bool invalid) {
    ...
    if (timeSynced) {
        time_t now = time(nullptr);
        if (now > 0) {
            unsigned long ntpTime = (unsigned long)now;
